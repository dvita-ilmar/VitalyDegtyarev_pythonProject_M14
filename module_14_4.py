...
# Запрос списка продуктов из БД бота
#products = get_all_products()
# Разместил его непосредственно в функции get_buying_list(), вопреки условию:
# мне показалось разместить непосредственно в функции "Купить" запрос к БД продуктов эффективнее, чем в начале работы бота,
# т.к. пользователь может и не захотеть просмотреть список товаров - чтобы зря не осуществлять операции с БД ?
# Товаров также 4 - т.к. для большего (меньшего) количества товаров пока непонятно как сделать инлайн-меню с переменным
# количеством кнопок выбора - соответствующим количеству товара?
